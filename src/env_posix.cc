...
ThreadPoolEnv::ThreadPoolEnv()
        : thread_pools_(Priority::TOTAL) // 默认创建4个线程池
{
    ThreadPoolImpl::PthreadCall("mutex_init", pthread_mutex_init(&mu_, nullptr));// 初始化互斥锁
    for (int pool_id = 0; pool_id < Env::Priority::TOTAL; ++pool_id) { // 遍历每一个线程池
        thread_pools_[pool_id].SetThreadPriority(static_cast<Env::Priority>(pool_id)); // 为每一个线程池设置优先级
        // 这允许稍后初始化每个线程的 thread-local-env。
        thread_pools_[pool_id].SetHostEnv(this); // 为每个线程池设置自己的运行环境
    }
}
...
Env *Env::Default() {
    ...
    //
    // 以下函数调用在静态 default_env 之前初始化 ThreadLocalPtr 的单例。
    // 这保证了 default_env 总是在 ThreadLocalPtr 单例被破坏之前被破坏，因为 C++ 保证静态变量的破坏与其构造的顺序相反。
    //
    ...
    // 由于静态成员的析构顺序与它们的构造顺序相反，因此在此处调用可确保静态 ThreadPoolEnv 的析构函数首先执行，然后是 ThreadLocalPtr 的单例。

    static ThreadPoolEnv default_env; // 自动调用构造函数，完成线程池的构造，线程池是一个静态类
    return &default_env;
}
