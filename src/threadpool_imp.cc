...
private:
    ...
    BGQueue queue_;                             // 保留线程池中调度的线程的相关信息

    std::mutex mu_;
    std::condition_variable bgsignal_;          // 条件变量，唤醒正在睡眠的线程
    std::vector<std::thread> bgthreads_;        // 存储线程的动态数组，保存需要调度的线程
};
